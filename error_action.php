...
if (isset($_POST['badge_entree']) and isset($_POST['badge_sortie']) and isset($_POST['id_badge'])){
    $VerifExist = $db->query("SELECT * FROM badgeuse_badge WHERE id_badge = ".securisation($_POST['id_badge']))->numRows();
    if ($VerifExist >0) {
        $date_debut_obj = new DateTime($_POST['badge_entree']);
        $date_fin_obj = new DateTime($_POST['badge_sortie']);
        if (!($date_debut_obj >= $date_fin_obj)) {
            ...
            $updateBadge = $db->query("UPDATE badgeuse_badge SET badge_date_entree = \"" . securisation($_POST['badge_entree']) . "\" WHERE id_badge = " . securisation($_POST['id_badge']));
            $updateBadge = $db->query("UPDATE badgeuse_badge SET badge_date_sortie = \"" . securisation($_POST['badge_sortie']) . "\" WHERE id_badge = " . securisation($_POST['id_badge']));
            $updateBadge = $db->query("UPDATE badgeuse_badge SET cron = 2 WHERE id_badge = " . securisation($_POST['id_badge']));
